...
function NurseMedication() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/user/get-users`);
  ...
  const handleAddMedication = async(e) =>{
    e.preventDefault() ;
    await axios.post(`${import.meta.env.VITE_API_URL}/user/add-medications/${changePatient}`,{name ,  frequency , dosage})
    .then((res) =>{
      Swal.fire({
        title: "Success",
        icon: "success",
